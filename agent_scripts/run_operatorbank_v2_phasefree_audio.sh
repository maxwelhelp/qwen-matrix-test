...
bash agent_scripts/patch_operatorbank_v2_plus1.sh || echo "[phasefree] main opv2 patch already applied or partially applied; continuing"
bash agent_scripts/patch_operatorbank_v2_hotfix.sh
bash agent_scripts/patch_phasefree_similarity.sh

STAMP="$(date +%Y%m%d_%H%M%S)"
REPORT_DIR="agent_reports/operatorbank_v2_phasefree_${STAMP}"
LATEST_LINK="agent_reports/latest_operatorbank_v2_phasefree"
mkdir -p "$REPORT_DIR"
rm -f "$LATEST_LINK"
ln -s "operatorbank_v2_phasefree_${STAMP}" "$LATEST_LINK"
...
PHASE_PRIOR_STRENGTH="${PHASE_PRIOR_STRENGTH:-0.0}"
...
printf '\n[phasefree] repo=%s\n' "$ROOT"
printf '[phasefree] report_dir=%s\n' "$REPORT_DIR"
printf '[phasefree] modes=%s epochs=%s lr=%s amp=%s\n' "$AUDIO_MODES" "$EPOCHS_AUDIO" "$AUDIO_LR" "$AMP_AUDIO"
printf '[phasefree] architecture: layers=%s steps=%s blocks=%s K=%s memory=%s global=%s dim=%s\n' "$LAYERS" "$STEPS" "$BLOCKS" "$PRIMITIVE_SLOTS" "$MEMORY_CELLS" "$GLOBAL_CELLS" "$DIM"
printf '[phasefree] phase_prior_strength=%s lambda_layer_sim=%s lambda_step_sim=%s\n\n' "$PHASE_PRIOR_STRENGTH" "$LAMBDA_LAYER_SIM" "$LAMBDA_STEP_SIM"

git rev-parse --short HEAD 2>/dev/null | sed 's/^/[phasefree] git_head=/' || true

for MODE in $AUDIO_MODES; do
  OUT="matrix_program_core/runs/operatorbank_v2_phasefree_${STAMP}/${MODE}"
  mkdir -p "$OUT"
  CKPT_ARGS=()
  ...
  python matrix_program_core/transfer_audio_assembler.py \
    ...
    --phase-prior-strength "$PHASE_PRIOR_STRENGTH" \
...
rd = Path(sys.argv[1])
print("# OperatorBankV2 phase-free summary")
for f in sorted(rd.glob("audio_*_final_report.json")):
    ...
    if m.exists():
        ...
        if rows:
            keys = ["epoch","train_ce","train_acc","val_acc","best_acc","class_read_div","slot_div","layer_sim","step_sim","primitive_balance","cell_balance","entropy_band","step_alive_budget","phase_balance"]
            print("last_focus:", {k: rows[-1].get(k) for k in keys})
PY

REPORT="$REPORT_DIR/REPORT_TO_CHATGPT.txt"
{
  echo "# REPORT_TO_CHATGPT OperatorBankV2 phase-free"
  echo
  ...
  echo "LAYERS=$LAYERS"; echo "STEPS=$STEPS"; echo "PHASE_PRIOR_STRENGTH=$PHASE_PRIOR_STRENGTH"
  echo "LAMBDA_LAYER_SIM=$LAMBDA_LAYER_SIM"; echo "LAMBDA_STEP_SIM=$LAMBDA_STEP_SIM"
  echo
...
printf '\n[phasefree] done. Paste this into ChatGPT:\n'
printf 'cat %s\n' "$REPORT"
