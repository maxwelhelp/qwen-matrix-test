...
REPORT_DIR="agent_reports/taskctx_v2_split_fixed_${STAMP}"
LATEST_LINK="agent_reports/latest_taskctx_v2_split_fixed"
mkdir -p "$REPORT_DIR"
rm -f "$LATEST_LINK"
ln -s "taskctx_v2_split_fixed_${STAMP}" "$LATEST_LINK"

LOG="$REPORT_DIR/full_run.log"
exec > >(tee -a "$LOG") 2>&1

PRETRAIN_OUT="matrix_program_core/runs/taskctx_v2_fixed_pretrain_${STAMP}"
AUDIO_ROOT="matrix_program_core/runs/taskctx_v2_fixed_audio_${STAMP}"
...
printf '\n[fixed-v2] repo=%s\n' "$ROOT"
printf '[fixed-v2] report_dir=%s\n' "$REPORT_DIR"
printf '[fixed-v2] pretrain_epochs=%s audio_epochs=%s modes=%s train_n=%s val_n=%s\n' "$EPOCHS_PRETRAIN" "$EPOCHS_AUDIO" "$AUDIO_MODES" "$TRAIN_N" "$VAL_N"
printf '[fixed-v2] export_dir=%s\n\n' "$EXPORT_ROOT"

git rev-parse --short HEAD 2>/dev/null | sed 's/^/[fixed-v2] git_head=/' || true

printf '\n[fixed-v2] multitask pretrain base skill\n'
...
python matrix_program_core/checkpoint_split.py --input "$PRETRAIN_CKPT" --out-dir "$EXPORT_ROOT" --tag pretrain
...
for MODE in $AUDIO_MODES; do
  OUT="$AUDIO_ROOT/$MODE"
  printf '\n[fixed-v2] live audio transfer mode=%s\n' "$MODE"
...
print("# task context v2 fixed split summary")
...
{
  echo "# REPORT_TO_CHATGPT taskctx v2 fixed split"
  echo
  echo "## git"
...
printf '\n[fixed-v2] done. Paste this into ChatGPT:\n'
printf 'cat %s\n' "$REPORT"
printf '\n[fixed-v2] local .pt split packages are in: %s\n' "$EXPORT_ROOT"
