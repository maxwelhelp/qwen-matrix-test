...
REPORT_DIR="agent_reports/assembler_multitask_fast_${STAMP}"
LATEST_LINK="agent_reports/latest_assembler_multitask_fast"
mkdir -p "$REPORT_DIR"
rm -f "$LATEST_LINK"
ln -s "assembler_multitask_fast_${STAMP}" "$LATEST_LINK"

LOG="$REPORT_DIR/full_run.log"
exec > >(tee -a "$LOG") 2>&1

PRETRAIN_OUT="matrix_program_core/runs/assembler_multitask_fast_${STAMP}"
AUDIO_ROOT="matrix_program_core/runs/audio_multitask_fast_${STAMP}"
...
printf '\n[multitask-fast] repo=%s\n' "$ROOT"
printf '[multitask-fast] report_dir=%s\n' "$REPORT_DIR"
printf '[multitask-fast] pretrain_epochs=%s audio_epochs=%s modes=%s train_n=%s val_n=%s log_every=%s\n' "$EPOCHS_PRETRAIN" "$EPOCHS_AUDIO" "$AUDIO_MODES" "$TRAIN_N" "$VAL_N" "$LOG_EVERY"
printf '[multitask-fast] tasks=%s\n' "$TASK_FAMILIES"
printf '[multitask-fast] solutions=%s\n\n' "$SOLUTION_FAMILIES"

git rev-parse --short HEAD 2>/dev/null | sed 's/^/[multitask-fast] git_head=/' || true

printf '\n[multitask-fast] run cached multitask assembler pretrain\n'
...
if [[ ! -f "$ASSEMBLER_CKPT" ]]; then
  echo "[multitask-fast][ERROR] missing assembler checkpoint: $ASSEMBLER_CKPT" >&2
  exit 2
fi
...
printf '\n[multitask-fast] transfer checkpoint: %s\n' "$ASSEMBLER_CKPT"
for MODE in $AUDIO_MODES; do
  OUT="$AUDIO_ROOT/$MODE"
  printf '\n[multitask-fast] run audio transfer mode=%s\n' "$MODE"
...
printf '\n[multitask-fast] build summary\n'
python - "$REPORT_DIR" <<'PY' | tee "$REPORT_DIR/summary.txt"
import csv, json, sys
from pathlib import Path
rd = Path(sys.argv[1])
print("# assembler multitask fast summary")
...
{
  echo "# REPORT_TO_CHATGPT assembler multitask fast"
  echo
  echo "## git"
...
printf '\n[multitask-fast] done. Paste this into ChatGPT:\n'
printf 'cat %s\n' "$REPORT"
printf '\n[multitask-fast] no .pt files copied to report_dir. PT checkpoints stayed under runs.\n'
find "$REPORT_DIR" -maxdepth 1 -type f -printf '[multitask-fast] report file: %p\n'
